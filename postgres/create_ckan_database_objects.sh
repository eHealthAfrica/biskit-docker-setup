...
psql <<- EOSQL
   CREATE ROLE $CKAN_DB_USER
   WITH LOGIN
        PASSWORD '${CKAN_DB_USER_PASS}'
        NOSUPERUSER
        NOCREATEDB
        NOCREATEROLE;
EOSQL
...
if [ -n "$CKAN_DATASTORE_WRITE_URL" ]; then
    ...
    if [ "$CKAN_DATASTORE_RW_DB_USER" != "$CKAN_DB_USER" ]; then
        psql <<- EOSQL
        CREATE ROLE $CKAN_DATASTORE_RW_DB_USER
        WITH LOGIN
             PASSWORD '${CKAN_DATASTORE_RW_DB_USER_PASS}'
             NOSUPERUSER
             NOCREATEDB
             NOCREATEROLE;
EOSQL
    ...
    psql <<- EOSQL
   CREATE ROLE $CKAN_DATASTORE_RO_DB_USER
   WITH LOGIN
        PASSWORD '${CKAN_DATASTORE_RO_DB_USER_PASS}'
        NOSUPERUSER
        NOCREATEDB
        NOCREATEROLE;
EOSQL
...
ALTER VIEW "_table_metadata" OWNER TO $CKAN_DATASTORE_RW_DB_USER;
